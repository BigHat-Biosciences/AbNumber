...
def _anarci_align(sequence, scheme, allowed_species, assign_germline=False) -> List[Tuple]:
    ...
    all_numbered, all_ali, all_hits = anarci(
        [("id", sequence)], scheme=scheme, allowed_species=allowed_species, assign_germline=assign_germline
    )
            ...
            for (num, letter), aa in positions:
                if str(num) == "61" and str(letter) == "A":
                    raise NotImplementedError(
                        f'Cannot parse sequence "{sequence}", '
                        f"ANARCI numbering of IMGT position 61A is currently broken: "
                        f"https://github.com/oxpig/ANARCI/issues/14"
                    )
        aa_dict = {
            Position(chain_type=chain_type, number=num, letter=letter, scheme=scheme): aa
            for (num, letter), aa in positions
            if aa != "-"
        }
        tail = sequence[end + 1 :]
...
SUPPORTED_CDR_DEFINITIONS = ["imgt", "chothia", "kabat", "north", "bighat"]
...
SCHEME_BORDERS = {
    ...
    "bighat_H": [26, 35, 50, 65, 98, 106, 114],
}
